...
async function addPlayerStats(item) {
    ...
    const id = `${item.player_id}-${item.season_year}-${league}`;

    await pool.promise().query(
        `INSERT INTO player_stats_by_year (
      id, player_id, season_year, games_played,
      efficiency_possession_impact_quotient, total_points, total_rebounds, total_assists,
      ...
      possessions, points_against, seasonal_epiq, epiq_per_game,
    ...
    `,
        [
            id, item.player_id, item.season_year, item.games_played,
            item.efficiency_possession_impact_quotient, item.total_points, item.total_rebounds, item.total_assists,
            item.total_steals, item.total_blocks, item.total_turnovers, item.total_fouls,
            item.field_goals_made, item.field_goals_attempted, item.three_points_made, item.three_points_attempted,
            item.free_throws_made, item.free_throws_attempted, item.offensive_rebounds, item.defensive_rebounds,
            item.possessions, item.points_against, item.seasonal_epiq, item.epiq_per_game,
            item.team, item.position, league
        ]
    );
}
...
async function updatePlayerStats(item) {
    ...
    WHERE id = ?`,
        [
            ...
            item.total_points, item.total_rebounds, item.total_assists,
            item.total_steals, item.total_blocks, item.total_turnovers, item.total_fouls,
            item.field_goals_made, item.field_goals_attempted,
            item.three_points_made, item.three_points_attempted,
            item.free_throws_made, item.free_throws_attempted,
            item.offensive_rebounds, item.defensive_rebounds,
            item.possessions, item.points_against,
            item.seasonal_epiq, item.epiq_per_game,
            item.team, item.position, league,
            id
        ]
    );
